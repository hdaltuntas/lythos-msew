#!/usr/bin/env python3
"""Run Lythos MSEW without installing it.

    python main.py                         opens the interface in a browser
    python main.py run project.msew        analyses a project file
    python main.py heights project.msew    runs the height study of a project file
...
nothing installed beyond NumPy, Matplotlib and reportlab.
"""
...
REQUIRED = ["numpy", "matplotlib", "reportlab"]
...
def explain(missing: list) -> None:
    print("Lythos MSEW needs these Python packages, which are not installed:\n",
          file=sys.stderr)
    for name in missing:
...
def main() -> int:
    if sys.version_info < (3, 10):
        print(f"Lythos MSEW needs Python 3.10 or later; this is "
    ...
    if gaps:
        explain(gaps)
        return 1
    from lythosmsew.cli import main as run_cli

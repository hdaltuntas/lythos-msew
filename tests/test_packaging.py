...
import lythosmsew
from lythosmsew import cli, forms
from lythosmsew.web import server
...
def test_the_version_is_the_same_in_the_package_and_the_distribution():
    ...
    assert declared.group(1) == lythosmsew.__version__


def test_the_static_files_are_declared_as_package_data():
    assert '"lythosmsew.web" = ["static/*"]' in pyproject()
...
def test_the_console_script_points_at_something_that_exists():
    assert 'lythos-msew = "lythosmsew.cli:main"' in pyproject()
...
def test_the_dependencies_are_the_ones_the_program_imports():
    ...
    package = os.path.join(ROOT, "lythosmsew")
...
def test_the_command_line_writes_runs_and_studies_a_project(tmp_path, capsys):
    path = tmp_path / "p.msew"
    assert cli.main(["example", "-o", str(path)]) == 0
    assert json.loads(path.read_text(encoding="utf-8"))["format"] == "lythos-msew"
    capsys.readouterr()
    assert cli.main(["run", str(path), "--lang", "tr"]) == 0
    assert "DONATILI ZEMİN DUVAR SONUÇLARI" in capsys.readouterr().out
    assert cli.main(["heights", str(path), "-o", str(tmp_path / "h.csv")]) == 0
    assert "HEIGHT STUDY" in capsys.readouterr().out
    assert (tmp_path / "h.csv").stat().st_size > 500


def test_the_bare_command_starts_the_interface(monkeypatch):
    """`lythos-msew` with no subcommand is the documented way to start it."""
    started = {}

    def fake_serve(host, port, open_browser, lang):
        started.update(host=host, port=port, open_browser=open_browser, lang=lang)

    import lythosmsew.web.server as server_module
...
def test_a_refused_analysis_prints_a_message_not_a_traceback(tmp_path, capsys):
    ...
    values["backslope"] = 40.0                     # steeper than the retained fill
    path = tmp_path / "impossible.msew"
    path.write_text(json.dumps(forms.project_file(values)), encoding="utf-8")
    assert cli.main(["run", str(path)]) == 1
    captured = capsys.readouterr()
    assert "Lythos MSEW:" in captured.err
    assert "backslope" in captured.err
    assert "Traceback" not in captured.err


def test_a_missing_project_file_is_reported_plainly(tmp_path, capsys):
    assert cli.main(["run", str(tmp_path / "nothing.msew")]) == 1
